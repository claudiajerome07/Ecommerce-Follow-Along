...
async function GetuserOrders(req,res) {
    ...
    try{
        ...
        const orders = await OrderModel.find({ user: userId, orderStatus: { $ne: 'Cancelled' } }, { orderStatus: 1, orderItems: 1 }).populate('orderItems')
...
async function CancelOrder(req,res){
    ...
    try{
        ...
        if(!mongoose.Types.ObjectId.isValid(orderId)){
            return res.status(400).send({ message: 'Invalid order Id', success: false })
        }

        await OrderModel.findByIdAndUpdate({_id:orderId},{orderStatus:'Cancelled'},{new:true})
